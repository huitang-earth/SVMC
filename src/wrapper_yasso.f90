...
public get_params
public decompose
public initialize_yasso
public initialize_yasso_totc
public average_met
...
contains
  ...
  !**************
  ! What's the pupose of this subroutine?
  !**************
  ...
  subroutine initialize_yasso(soilcn_state)
   !param, flux_leafc_day, flux_rootc_day, flux_nitr_day, &
   !    tempr_c, precip_day, tempr_ampl, totc_min, cstate, nstate)
    ...
    ! Carbon
    neg_c_input_yr = -(flux_leafc_day * awenh_leaf + flux_rootc_day * awenh_fineroot) * days_yr
    call evaluate_matrix_mean_tempr(param_y20_map, tempr_c, precip_day * days_yr, tempr_ampl,  matrix)
    ! Solve the equilibrium condition Ax + b = 0
    call solve(matrix, neg_c_input_yr, soilcn_state%cstate)
    totc = sum(soilcn_state%cstate)
    !print *, 'totc before adjust', totc, totc_min
    if (totc_min > 0.0 .and. totc < totc_min) then
       soilcn_state%cstate(5) = soilcn_state%cstate(5) + totc_min - totc
    end if
    !print *, 'totc after adjust', sum(cstate)

    ! Nitrogen
    ! Hui: not needed for the moment.
    ...
    !     matrix, &
    !     nstate)
  end subroutine initialize_yasso
  ...
  subroutine initialize_yasso_totc(param, totc, cn_input, fract_root_input, fract_legacy_soc, &
  ...
  end subroutine initialize_yasso_totc
